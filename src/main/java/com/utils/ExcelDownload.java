    ...
    public static <T> void export(String excelName, List<T> list, LinkedHashMap<String, String> fieldMap, HttpServletResponse response){
        ...
        try {
            response.setHeader("Content-disposition", "attachment; filename="
                    +new String(excelName.getBytes("GBK"), "ISO-8859-1")  + ".xls");
        ...
        for (int index = 0; index < list.size(); index++) {
            ...
            for (int i = 0; i < enFields.length; i++) {
                ...
                if(objValue==null) {
                    ...
                    if(objValue.getClass().getName().equals("java.lang.Integer")) {        //判断数据输出类型

                                                       //一定要先设置单元格数据类型
                        row.createCell(i).setCellValue( Double.parseDouble(objValue.toString()));//再输出数据
                    }else {
